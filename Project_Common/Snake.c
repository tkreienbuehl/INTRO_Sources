...
static int random(int min, int max) {
  ...
  if (cnt<(TickType_t)min) {
    cnt = (TickType_t)min;
  }
  if (cnt>(TickType_t)max) {
    cnt = (TickType_t)(max/2);
  }
  return (int)cnt;
}
...
static void showPause(void) {
  ...
  x = (FDisp1_PixelDim)(GDisp1_GetWidth()-FDisp1_GetStringWidth((unsigned char*)"Pause", font, NULL))/2; /* center text */
  y = 0;
  FDisp1_WriteString((unsigned char*)"Pause", GDisp1_COLOR_BLACK, &x, &y, font);

  x = 0;
  y += (FDisp1_PixelDim)(totalHeight);
...
static void moveSnake(void) {
  /* LEFT */
  if(EVNT_EventIsSet(EVNT_SNAKE_LEFT) && !right) {
	  EVNT_ClearEvent(EVNT_SNAKE_LEFT);
  ...
  /* RIGHT */
  if(EVNT_EventIsSet(EVNT_SNAKE_RIGHT) && !left) {
	  EVNT_ClearEvent(EVNT_SNAKE_RIGHT);
    if((xSnake > 0 || xSnake <= GDisp1_GetWidth() - xSnake)) {
      direc(RIGHT);
    }
    return;
  ...
  /* UP */
  if(EVNT1_GetClearEvent(EVNT_SNAKE_UP) && !down) {
	  EVNT_ClearEvent(EVNT_SNAKE_UP);
  ...
  /* DOWN */
  if(EVNT1_GetClearEvent(EVNT_SNAKE_DOWN) && !up) {
	  EVNT_ClearEvent(EVNT_SNAKE_DOWN);
    if((ySnake > 0 || ySnake <= GDisp1_GetHeight() - ySnake)) {
      direc(DOWN);
    }
    return;
  ...
  /* START/PAUSE */
  if(EVNT1_GetClearEvent(EVNT_SNAKE_START_PAUSE)) {
    showPause();
  }
}
...
static void gameover(void) {
  ...
  x = (GDisp1_GetWidth()-FDisp1_GetStringWidth((unsigned char*)"(Press Center Btn)", font, NULL))/2; /* center text */
  y += totalHeight;
  FDisp1_WriteString((unsigned char*)"(Press Center Btn)", GDisp1_COLOR_BLACK, &x, &y, font);
  GDisp1_UpdateFull();
  delay(4000);
  waitAnyButton();
...
#endif /* PL_CONFIG_HAS_SNAKE_GAME */
